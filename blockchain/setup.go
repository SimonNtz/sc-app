...
func (setup *FabricSetup) InstallAndInstantiateCC(boostrapVars [][]byte) error {

	// Create the chaincode package that will be sent to the peers
	// ccPkg, err := packager.NewCCPackage(setup.ChaincodePath, setup.ChaincodeGoPath)
	ccPkg := resource.CCPackage{peer.ChaincodeSpec_GOLANG, setup.ChaincodeCode,}
	if err != nil {
	...
	resp, err := setup.admin.InstantiateCC(setup.ChannelID, resmgmt.InstantiateCCRequest{
		...
		Version: "0",
		Args: append([][]byte{[]byte("init")}, boostrapVars...),
		Policy: ccPolicy,
	})
