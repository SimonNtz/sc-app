...
func (t *HeroesServiceChaincode) Init(stub shim.ChaincodeStubInterface) pb.Response {
	fmt.Println("########### Smart-Consent Init ###########")

	// Get the function and arguments from the request
	function, args := stub.GetFunctionAndParameters()

	if len(args) < 1 {
		return shim.Error("No argument when calling Init")
	}
	...
	for _, arg := range args {
		err := stub.PutState(arg, []byte("Not authentified"))
		if err != nil {
...
func (t *HeroesServiceChaincode) invoke(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if err != nil {
		return shim.Error(fmt.Sprintf("Failed to get state of %s", args[1]))
	}
	...
	if len(args) == 3 {
		...
		if err != nil {
			return shim.Error(fmt.Sprintf("Failed to update state of %s", args[1]))
		}
